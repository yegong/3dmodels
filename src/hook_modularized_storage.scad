...
module hook() {
    peg1_length = 6;
    peg2_length = 3;
    peg1_angle = atan((peg_offset-peg_radius)/(peg2_length+peg_radius));
    
    // Peg 1
    up(peg_offset/2)
    ycyl(h = peg_radius+board_thickness, r = peg_radius, anchor=FRONT) {
        
        position(BACK)
        xrot(-peg1_angle)
        ycyl(h = peg1_length, r = peg_radius, anchor=FRONT) {
            position(BACK) sphere(r = peg_radius);
            position(FRONT) sphere(r = peg_radius+0.05);
        }
